...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Detect touch-only devices or low power devices
    if (typeof window !== "undefined" && window.matchMedia("(max-width: 768px)").matches) {
      // Disable on mobile/tablet to ensure buttery smooth performance
      return;
    }
    ...
    function getWebGLContext(canvasElement: HTMLCanvasElement) {
      ...
      if (isWebGL2) {
        formatRGBA = getSupportedFormat(glContext, (glContext as any).RGBA16F, glContext.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(glContext, (glContext as any).RG16F, (glContext as any).RG, halfFloatTexType);
        formatR = getSupportedFormat(glContext, (glContext as any).R16F, (glContext as any).RED, halfFloatTexType);
      } else {
    ...
    function compileShader(type: number, source: string, keywords?: string[]) {
      const updatedSource = addKeywords(source, keywords);
    ...
    function clickSplat(pointer: PointerPrototype) {
      ...
      const r = color[0] * 10.0;
      const g = color[1] * 10.0;
      const b = color[2] * 10.0;
      const dx = 10 * (Math.random() - 0.5);
      const dy = 30 * (Math.random() - 0.5);
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, [r, g, b]);
    }

    function splat(x: number, y: number, dx: number, dy: number, colorArr: [number, number, number]) {
      splatProgram.bind();
      ...
      gl.uniform3f(splatProgram.uniforms.color, colorArr[0], colorArr[1], colorArr[2]);
      blit(dye.write);
      dye.swap();
    }
